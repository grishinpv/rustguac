...
export function EntryModal({
  ...
  const [remoteApp, setRemoteApp] = useState(false)
  ...
  return (
    <div
      className="modal-overlay active fixed inset-0 z-[90] flex items-start justify-center overflow-y-auto bg-black/50 p-4 py-10"
      role="presentation"
      onMouseDown={(e) => e.target === e.currentTarget && onClose()}
    >
      <div className="modal my-4 w-full max-w-[640px] rounded border p-5" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
        <h3 className="mt-0">{title}</h3>
        <label className="mt-2 block text-xs uppercase" style={{ color: 'var(--text-muted)' }}>
          Entry name
          <input className="mt-1 block w-full" value={entryName} disabled={!!editTarget && editTarget.mode === 'edit'} onChange={(e) => setEntryName(e.target.value)} pattern="[a-zA-Z0-9_.-]+" maxLength={64} />
        </label>
        <label className="mt-2 block text-xs uppercase" style={{ color: 'var(--text-muted)' }}>
          Display name (optional)
          <input className="mt-1 block w-full" value={f.displayName} onChange={(e) => set({ displayName: e.target.value })} />
        </label>
        <label className="mt-2 block text-xs uppercase" style={{ color: 'var(--text-muted)' }}>
          Type
          <select
        ...
        {f.type === 'rdp' ? (
          <Collapse label="RemoteApp (RAIL)" open={remoteApp} onToggle={() => setRemoteApp(!remoteApp)}>
            <label className="block text-xs uppercase" style={{ color: 'var(--text-muted)' }}>
              Remote app
        ...
        <Collapse label="Recording settings" open={recording} onToggle={() => setRecording(!recording)}>
          <label className="flex items-center gap-2 text-sm">
        ...
        {f.type === 'rdp' ? (
          <Collapse label="Video performance" open={video} onToggle={() => setVideo(!video)}>
            <label className="flex items-center gap-2 text-sm">
        ...
        <div className="mt-3 space-y-2 border-t pt-2" style={{ borderColor: 'var(--border)' }}>
          <label className="flex items-center gap-2 text-sm">
        ...
        {f.type === 'ssh' || f.type === 'rdp' || f.type === 'vnc' || f.type === 'web' ? (
          <Collapse label="SSH Tunnel / Jump Hosts" open={tunnel} onToggle={() => setTunnel(!tunnel)}>
            ...
            {hopRows.map((hop, i) => (
              <div key={i} className="mb-2 border p-2" style={{ borderColor: 'var(--border)', background: 'var(--bg)' }}>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <input className="min-w-0 flex-1 font-mono" placeholder="bastion.example.com" value={hop.hostname} onChange={(e) => {
                ...
                {hop.expanded ? (
                  <div className="mt-2 space-y-2 border-t pt-2" style={{ borderColor: 'var(--border)' }}>
                    <label className="block text-xs" style={{ color: 'var(--text-muted)' }}>
                      Port
                      <input type="number" className="mt-1 block w-full" value={hop.port} onChange={(e) => {
                        const n = hopRows.slice()
                    ...
                    <label className="block text-xs" style={{ color: 'var(--text-muted)' }}>
                      Username
                      <input className="mt-1 block w-full" value={hop.username} onChange={(e) => {
                        const n = hopRows.slice()
                    ...
                    <label className="block text-xs" style={{ color: 'var(--text-muted)' }}>
                      Password
                      <input type="password" className="mt-1 block w-full" value={hop.password} onChange={(e) => {
                        const n = hopRows.slice()
                    ...
                    <label className="block text-xs" style={{ color: 'var(--text-muted)' }}>
                      Private key
                      <textarea className="mt-1 block w-full font-mono text-sm" rows={2} value={hop.private_key} onChange={(e) => {
                        const n = hopRows.slice()
                ...
                <button type="button" className="btn-small mt-1 w-full border-t pt-1" style={{ borderColor: 'var(--border)' }} onClick={() => {
                  const n = hopRows.slice()
            ...
            <button type="button" className="btn-add mt-2" onClick={() => setHopRows([...hopRows, { hostname: '', port: 22, username: '', password: '', private_key: '', expanded: true }])}>
              + Add Jump Host
            </button>
          </Collapse>
        ) : null}
        ...
        {editTarget && editTarget.mode === 'edit' ? (
          <label className="mt-3 block text-xs uppercase" style={{ color: 'var(--text-muted)' }}>
