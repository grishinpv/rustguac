...
Guacamole.SessionRecording = function SessionRecording(source, refreshInterval) {
    ...
    /**
     * Returns minimal frame metadata (relative timestamp and byte size)
     * for all frames parsed so far. Frame byte size correlates with screen
     * activity — idle frames are tiny, active frames are large.
     *
     * @returns {!Array.<{timestamp: number, size: number}>}
     */
    this.getFrames = function getFrames() {
        return frames.map(function(f) {
            return {
                timestamp: toRelativeTimestamp(f.timestamp),
                size: f.end - f.start
            };
        });
    };
